...
Table~\ref{tab:loss-full} gives the complete record for the six runs under seed 0, so that the
shape of the curves discussed in Section~3.2 can be inspected directly. The other two seeds follow
the same trajectory and are not reproduced; their final values appear in Table~\ref{tab:fid} and
the underlying record is available in the accompanying repository.
